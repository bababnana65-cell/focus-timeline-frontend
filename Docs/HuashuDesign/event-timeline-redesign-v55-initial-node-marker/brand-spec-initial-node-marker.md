# 事件时间轴 App · Initial Node Marker v55 视觉规范
> 日期：2026-04-30  
> 范围：基于 v54 的时间轴初始节点标记优化，不修改 Flutter 代码  
> 关键调整：重新设计第一个节点旁边的圆形图标，并避免绿色轨道线穿过初始节点

## 本版调整

v55 只调整时间轴详情页第一个节点：

- `.timeline .node:first-of-type .count-dot` 改为 `始` 字圆形标记，让用户知道这是初始节点。
- 初始节点标记保持圆形，尺寸略大于普通节点，但不改变右侧节点卡片、日期胶囊和文字位置。
- 初始节点标记增加同背景色留白，遮住背后的绿色轨道线，避免轨道穿过图标。
...
- `event-timeline-initial-node-marker.html`
