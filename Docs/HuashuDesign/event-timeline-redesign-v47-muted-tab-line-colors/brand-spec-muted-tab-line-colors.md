# 事件时间轴 App · Restored Icon Shadow v46 视觉规范
> 日期：2026-04-29  
> 范围：基于 v45 的底部时间轴 Tab 图标还原和配色调整，不修改 Flutter 代码  
> 关键调整：选中/未选中图标同尺寸，恢复原始阴影和双层线条结构
...
- 选中图标配色：绿色主色块 `#DFF2C7`、绿色阴影 `#86A866`、珊瑚红线条 `#D95F4D / #F06F5D`。
- 未选中态保留阴影和双层线条结构，使用低对比灰绿配色，不抢当前页面主题。
...
- `.bottom .nav:nth-child(2).active .timeline-tab-mark .mark-accent`：`#D95F4D`。
...
- `event-timeline-restored-icon-shadow.html`
