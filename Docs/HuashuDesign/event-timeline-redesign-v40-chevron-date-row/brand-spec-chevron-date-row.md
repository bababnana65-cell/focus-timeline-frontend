# 事件时间轴 App · Chevron Date Row v40 视觉规范
> 日期：2026-04-29  
> 范围：基于 v39 的时间轴节点右侧箭头控件调整，不修改 Flutter 代码  
> 关键调整：保持文本框右边界一致，同时把箭头恢复为可识别的轻量圆形控件
...
- `.timeline .node .chev` 调整为 `22px × 22px`。
- 箭头按钮位置为 `right: 8px; top: 15px`，位于日期胶囊同一行右侧。
- 这样文本框仍然对齐，箭头也不会小到失去控件感。
...
- `event-timeline-chevron-date-row.html`
