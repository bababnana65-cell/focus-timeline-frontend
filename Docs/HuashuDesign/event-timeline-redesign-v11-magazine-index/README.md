# 事件时间轴 App 视觉重设计 · Magazine Index v11

本目录是 `huashu-design` 交付物，只做视觉设计，不修改 Flutter 业务代码。

## 本版方向

- 单独新建 v11 目录，没有覆盖 v10 或更早版本。
- 采用清爽杂志索引风：轻分隔线、小编号、状态标签和清楚文字层级。
- 取消票据切口和右侧票根视觉，避免装饰抢占新闻文字空间。
- 推荐、关注、时间轴都更像“专题索引 / 事件目录”，便于快速扫读。
- 不使用渐变色。
- 不使用不规则多边形色块。
...
- `event-timeline-magazine-index.html`：高保真移动端设计板。
- `brand-spec-magazine-index.md`：清爽杂志索引风视觉规范。
