# 事件时间轴 App 视觉重设计 · Map Markers v12

本目录是 `huashu-design` 交付物，只做视觉设计，不修改 Flutter 业务代码。

## 本版方向

- 单独新建 v12 目录，没有覆盖 v11 或更早版本。
- 采用地图标记风：专题、推荐和时间轴节点像一组路线标记。
- 使用小路线点、轻路径线、坐标式标签和站点感组织阅读。
- 图标保持低占比，主要空间仍留给新闻标题、摘要和最新动态。
- 不使用渐变色。
- 不使用不规则多边形色块。
...
- `event-timeline-map-markers.html`：高保真移动端设计板。
- `brand-spec-map-markers.md`：地图标记风视觉规范。
