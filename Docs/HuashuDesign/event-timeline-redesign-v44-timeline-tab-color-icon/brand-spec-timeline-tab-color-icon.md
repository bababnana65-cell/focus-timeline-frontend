# 事件时间轴 App · Timeline Tab Color Icon v44 视觉规范
> 日期：2026-04-29  
> 范围：基于 v43 的底部时间轴 Tab 视觉调整，不修改 Flutter 代码  
> 关键调整：时间轴 Tab 选中时去掉外层圆形背景，直接显示彩色图标
...
- 选中图标尺寸从 `42px` 收到 `38px`，未选中图标为 `34px`。
- 外层触控槽保持 `58px × 52px`，保证命中面积和左右 Tab 高度对齐。
- 选中图标配色：绿色主形 `#5A9D2C`、珊瑚橙笔触 `#F27861`、柔和暖底形 `#FBE5D7`。
...
- 图标选中尺寸为 `38px`，未选中尺寸为 `34px`，外层触控槽为 `58px × 52px`。
...
- `.timeline-tab-mark`：默认 `30px × 30px`，第二个 Tab 纯图标态未选中为 `34px × 34px`，选中为 `38px × 38px`。
...
- `.bottom .nav:nth-child(2).active .timeline-tab-mark .mark-main`：`#5A9D2C`。
- `.bottom .nav:nth-child(2).active .timeline-tab-mark .mark-accent`：`#F27861`。
- `.bottom .nav:nth-child(2).active .timeline-tab-mark .mark-blob`：`#FBE5D7`。
...
- `event-timeline-tab-color-icon.html`
