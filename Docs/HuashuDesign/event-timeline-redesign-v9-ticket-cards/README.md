# 事件时间轴 App 视觉重设计 · Ticket Cards v9

本目录是 `huashu-design` 交付物，只做视觉设计，不修改 Flutter 业务代码。

## 本版方向

- 单独新建 v9 目录，没有覆盖 v8 或更早版本。
- 采用卡片票据风：专题、推荐和时间轴节点像一张张轻票据。
- 使用右侧虚线票根、上下小切口、日期章和状态章表达“事件记录凭证”。
- 不使用渐变色。
- 不使用不规则多边形色块。
...
- `event-timeline-ticket-cards.html`：高保真移动端设计板。
- `brand-spec-ticket-cards.md`：卡片票据风视觉规范。
