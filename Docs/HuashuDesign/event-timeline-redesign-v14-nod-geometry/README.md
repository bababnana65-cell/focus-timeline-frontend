# 事件时间轴 App 视觉重设计 · NOD-inspired Geometry v14

本目录是 `huashu-design` 交付物，只做视觉设计，不修改 Flutter 业务代码。

## 本版方向

- 单独新建 v14 目录，没有覆盖 v13 或更早版本。
- 采用 NOD-inspired 几何阅读风：准确轮廓、节奏编号、小面积高能量颜色、中文标题张力。
- 这不是艺术海报化复刻，而是把 Nod Young 相关视觉语言转译成新闻阅读 UI。
- 图标和装饰保持低占比，主要空间仍留给新闻标题、摘要、日期和动态。
- 不使用渐变色。
- 不使用不规则多边形色块。
...
- `event-timeline-nod-geometry.html`：高保真移动端设计板。
- `brand-spec-nod-geometry.md`：NOD-inspired 几何阅读风视觉规范。
