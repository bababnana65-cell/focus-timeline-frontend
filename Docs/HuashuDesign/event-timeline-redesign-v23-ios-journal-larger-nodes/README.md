# 事件时间轴 App 视觉重设计 · iOS Journal Larger Nodes v23

本目录是 `huashu-design` 交付物，只做视觉设计，不修改 Flutter 业务代码。

## 本版方向

- 单独新建 v23 目录，没有覆盖 v22 或其他版本。
- 以 v22 为基础，只调整时间轴左侧流线上的节点圆点。
- 节点圆点从 20px 放大到 24px，数字字号同步略增。
- 圆点位置重新居中到流线上，保持节点流线干净。
- 保留 v22 的单行统计条和 v21 的上下日期标签结构。
- 不使用渐变色。
- 不使用不规则多边形色块。
...
- `event-timeline-ios-journal-larger-nodes.html`：高保真移动端设计板。
- `brand-spec-ios-journal-larger-nodes.md`：放大节点圆点视觉规范。
