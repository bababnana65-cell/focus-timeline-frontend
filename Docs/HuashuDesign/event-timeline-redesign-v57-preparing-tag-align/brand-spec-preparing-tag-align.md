# 事件时间轴 App · Preparing Tag Align v57 视觉规范
> 日期：2026-04-30  
> 范围：基于 v56 的时间轴准备中流程标签对齐优化，不修改 Flutter 代码  
> 关键调整：修正准备中流程右侧状态标签的底图和文字对齐

## 本版调整

v57 只调整时间轴准备中页面里的状态标签：

- `.step > .pill` 明确恢复为 `inline-flex`，避免被 `.step span` 普通说明文字规则覆盖。
- 右侧 `完成 / 进行中 / 等待` 统一为 `28px` 高度、`56px` 固定宽度。
- 标签文字 `line-height: 1`，横向和纵向都居中。
- 三个状态标签显式设置状态底色，避免被时间轴页面主题色冲淡后看起来像裸文字。
- 不改变准备中页面的信息、流程状态、进度条和业务结构。

## 继承调整

继承 v56 的时间轴初始节点和轨道起点：

- `.timeline .node:first-of-type .count-dot` 改为 `起` 字圆形标记，让用户知道这是时间线起点。
- 初始节点标记保持圆形，尺寸略大于普通节点，但不改变右侧节点卡片、日期胶囊和文字位置。
- `.timeline::after` 和 `.timeline::before` 的 `top` 下移到初始图标之后，绿色轨道可以贴近图标，但不会从上方冒出。
- 其他节点圆点、最新节点虚线、横向连接线、节点卡片和时间轴业务结构保持不变。

## 继承调整

继承 v55 的初始节点单独视觉标记，并在本版收起轨道上冒线。
...
- `event-timeline-preparing-tag-align.html`
