...
(async () => {
  ...
  const htmlPath = path.join(root, 'event-timeline-bottom-timeline-app-icon.html');
  ...
  const metrics = await page.evaluate(() => {
    ...
    return {
      timelineSelected: {
        ...
        firstLabelTop: Math.round(timelineTabs[0]?.getBoundingClientRect().top || 0),
        secondLabelTop: Math.round(timelineTabs[1]?.getBoundingClientRect().top || 0),
