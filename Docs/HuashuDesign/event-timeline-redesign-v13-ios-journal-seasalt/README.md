# 事件时间轴 App 视觉重设计 · iOS Journal Seasalt v13

本目录是 `huashu-design` 交付物，只做视觉设计，不修改 Flutter 业务代码。

## 本版方向

- 单独新建 v13 目录，没有覆盖 v8 或其他版本。
- 基于 v8 iOS Journal 风，只调整配色和状态色。
- 主色从偏绿纸感调整为海盐蓝白，阅读界面更清爽。
- 薄荷绿只用于准备中、路线进度等轻状态。
- 珊瑚色只用于新动态、错误、关注上限等提醒状态。
- 不使用渐变色。
- 不使用不规则多边形色块。
...
- `event-timeline-ios-journal-seasalt.html`：高保真移动端设计板。
- `brand-spec-ios-journal-seasalt.md`：iOS Journal 新配色视觉规范。
