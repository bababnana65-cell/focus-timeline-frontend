# 事件时间轴 App · Timeline Text Right Spacing v37 视觉规范
> 日期：2026-04-29  
> 范围：基于 v36 的时间轴节点文字右间距微调，不修改 Flutter 代码  
> 关键调整：收窄时间轴节点标题和摘要宽度，给右侧箭头留出稳定阅读空隙

## 本版调整

v37 只调整时间轴节点里的标题和摘要文本：

- `.timeline .node h3 / p` 宽度从 `264px` 调整为 `242px`。
- 保留左侧起点 `margin-left: 21px`，不改变日期、流线和节点卡片位置。
- 右侧不再压到箭头视觉通道，节点标题和摘要的右侧留白更接近“我的关注”卡片里的阅读文本。
- `.timeline .node .chev` 仍继承 v33：卡片内 `right: 10px`。
...
- `.timeline .node h3, .timeline .node p`：`width: min(242px, calc(100% - 45px))`。
...
- `event-timeline-timeline-text-right-spacing.html`
